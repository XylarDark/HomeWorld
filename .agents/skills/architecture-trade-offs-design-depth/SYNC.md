...
**Copies (byte-identical; no second body):** HomeWorld `.agents/skills/architecture-trade-offs-design-depth/SKILL.md` · Co Grok skill `architecture-trade-offs-design-depth`

## Checklist
1. Edit **writer** on DET only.
2. `git hash-object SKILL.md` → update **Canon blob** above.
3. Copy `SKILL.md` bytes unchanged → HW core path (same blob).
4. Refresh Co skill from writer bytes (no paraphrase).
5. Keep lean extras `architecture-tradeoffs` / `design-complexity` as optional A/B subsets — not a fork.
6. Never paste A–E body into `AGENTS.md` (pointer row only).
7. Verify DET blob == HW blob == Canon.
8. Stop — no pin bump and no core→extras move in this bite.