...
**Planetoid structure and progression (post-MVP):** The following is **vision only, not MVP scope**. A shape/method will be used to create **chunks that load as biomes**, so different sections of a planetoid can host different content. **Only harvestables** (trees, rocks, flowers/herbs, etc.) are **destroyable**; everything else is persistent. **Anything below ground** (subsurface) acts as a **portal to a dungeon map** — entering below-ground leads to a separate dungeon level. As you **conquer the planetoid and each biome**, you **unlock items**. Those items are used **back home at your spaceship** to **choose which biomes appear on your next planetoid**. This **biome-selection unlock loop** continues **throughout the game**: each conquered planetoid/biome feeds into what you can configure for the next run. See [PLANETOID_DESIGN.md](../PLANETOID_DESIGN.md) for proc-gen and chunk/biome loading (post-MVP).
...
> **Product canon (Docs/21 Reap & Sow, Lead `APPROVE RS STRATEGY` 2026-09-19):** For shipped MVP / VS_MVP product work, **day = reap** (collect / claim), **night = sow** (nurture / influence), and **astral combat = dream battles** that **heal and recruit** animals, spirits, and humanoids (**convert, do not kill**). Site kit and cross-bonus special site: [Docs/21_REAP_SOW.md](../../Docs/21_REAP_SOW.md) · executable tables: [Docs/01_GDD_MVP.md](../../Docs/01_GDD_MVP.md) §3.1. That framing **supersedes** “defeat/kill monsters” night flavor for product implementation. Long-form campaign color below still informs theme; where it conflicts with Docs/21, **Docs/21 + Docs/01 win**.
