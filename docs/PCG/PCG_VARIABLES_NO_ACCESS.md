...
**Version note:** UE 5.2–5.8. In UE 5.7+ the Get Landscape Data **actor selector** offers **By Tag** only (no By Class for actor). **WTR-C / UE 5.8.2 (2026-09-19):** re-ran `pcg_settings_introspect.py` → `Saved/pcg_settings_introspect_5.7.txt`. Still readable: `actor_selector` (`PCGActorSelectorSettings`), `component_selector`, `unbounded` (bool), `sampling_properties`. Still **no reliable Python write** path for By Tag name / Static Mesh Spawner mesh entries (same no-access row). Nondestructive PCG edit remains Editor-side; Epic PCGToolset AICallable is a future wrap candidate ([U58F_F](../../Docs/handoffs/U58F_F_MCP_DECISION.md)).
