...
**PA-E MRQ PIE void black sky, cabin lit — post-#175 DESKTOP (2026-09-22):** After framing fix, MRQ stills show **warm cabin** but **solid black sky** (no moon/stars/ambient). **Cause:** `MoviePipelinePIEExecutor` respawns PIE world; Editor **`stack_ok`** skipped reseed/reconfigure; SkyLight cubemap/atmo not refreshed (Atmosphere Sun Light + RecaptureSky). **Fix:** [vnp_night_tune_and_evidence.py](../Content/Python/vnp_night_tune_and_evidence.py) `apply_mrq_pie_homestead_night_stack` + per-shot `reapply_night_environment_for_mrq_shot` (`mrq_pie_shot=True`); MRQ `MoviePipelineConsoleVariableSetting` exposure clamps — **not** day phase.
