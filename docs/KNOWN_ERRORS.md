...
**PA-E MRQ framing ≠ Arrange (post-#172 DESKTOP, 2026-09-22):** Ray hit / live **CAM_*** relocate can PASS while stills show edge scrap — stale sequence **Transform** on possessable **CAM_*** overrides MRQ; extent offsets miss hero center. **Fix:** [pa_e_shotlist_common.py](../Content/Python/pa_e_shotlist_common.py) doc **CAM_Hero** / cabin-close meters + look_at dress centroid + `distance_band_ok`; [capture_shotlist_mrq.py](../Content/Python/capture_shotlist_mrq.py) **`PA_E_MRQ_{shot}`** spawned at resolved loc/rot, purge bindings, **add_possessable** only — not visual PASS until center-crop assert passes.
