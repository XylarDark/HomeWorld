...
**PA-E relocate + ray hit ≠ hero framing PASS — post-#172 DESKTOP (2026-09-22):** Arrange **`ready: true`**, **`forward_ray_hits_dress_aabb: true`**, but MRQ stills were mostly black with **fence/rock scrap on the right edge** (global mean ~8.7, center crop ~0.25). **Cause:** `_camera_pose_from_bounds` extent offsets placed the camera sideways so the forward ray grazed dress AABB while homestead mass was off-center. **Do not treat ray hit alone as visual PASS.** **Fix:** [pa_e_shotlist_common.py](../Content/Python/pa_e_shotlist_common.py) — shot1/2 **doc meter locations** (CAM_Hero / CAM_CabinClose) + **`find_look_at_rotation`** to **aim_bounds** dress centroid; **`aim_ok`** adds **`distance_band_ok`** per shot; hardened assert (center crop / bright-pixel fraction) remains authoritative for capture PASS.

**PA-E MRQ possessable CAM_* + stale sequence Transform ≠ Arrange pose — post-#172 DESKTOP (2026-09-22):** [capture_shotlist_mrq.py](../Content/Python/capture_shotlist_mrq.py) **`find_binding_by_name`** reused Level Sequence possessables for in-level **CAM_Hero** / **CAM_CabinClose**; **Transform tracks** on reused assets kept **old poses** so Arrange **`set_actor_location`** on the live actor did not change the MRQ frame (same fence/rock scrap on the right). **Do not treat live CAM relocate as MRQ framing PASS.** **Fix:** always **`_ensure_spawned_cine_camera`** at resolved Arrange **loc/rot** (`PA_E_MRQ_{shot_id}`); **purge bindings + strip Transform tracks** before camera cut; **`add_possessable`** on spawned camera only (no named-binding reuse).
