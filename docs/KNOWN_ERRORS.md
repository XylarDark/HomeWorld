...
- **PS-C `ready_for_ps_c` gate but PS_* `camera_missing` (2026-09-23, PS-C-CAM-002):** Stale **`Saved/ps_arrange_gate.json`** after level reload — JSON **`ready_for_ps_c: true`** without live **`PS_*`/`CAM_*`** actors. **Avoid:** **`_still_cam_labels_missing()`** → re-run **`arrange_ps_homestead.py`** when any **`STILL_CAM_LABELS`** absent (do not trust sidecar alone).
