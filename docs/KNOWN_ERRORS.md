...
**PA-E MRQ PIE void black sky, cabin lit — post-#175/#177 DESKTOP (2026-09-22):** MRQ stills: **lit cabin**, **RGB(0,0,0) sky**. **Cause:** `MoviePipelinePIEExecutor` renders a **PIE copy** — Editor-only TMP SkyAtmosphere/moon never transfer; `AtmosphereSunLightIndex 1` with a single directional skips atmosphere (use **0**); insane dress centroid (Z≈-3656) poisons moon aim. **Avoid:** Editor `stack_ok` alone; index 1-only; skip PIE-world apply. **Fix:** [vnp_night_tune_and_evidence.py](../Content/Python/vnp_night_tune_and_evidence.py) world-aware stack + `apply_mrq_pie_homestead_night_stack_in_render_world` from [capture_shotlist_mrq.py](../Content/Python/capture_shotlist_mrq.py) wait loop; centroid sanitize; MRQ `r.SupportSkyAtmosphere 1` — **not** day phase.
