...
**PA-E AutomationLibrary pretick writes near-black PNGs — post-#166 DESKTOP FAIL (2026-09-22, after ee32888):** MCP **`capture_shotlist_viewport.py`** pretick path **responsive**; **`Saved/pa_e_capture_report.json`** **`ok: false`** — Shot1 ~38KB **mean luminance ~0.0**, Shot2 ~143KB **mean ~0.3** (validation **`near_black`** / fail luminance gate). Files exist but are not lit shotlist evidence. **Fix (rung 1):** retire AL as PASS primary; implement **[capture_shotlist_mrq.py](../Content/Python/capture_shotlist_mrq.py)** — Movie Render Queue one-frame (`MoviePipelineOutputSetting.use_custom_playback_range`, deferred PNG pass, `MoviePipelinePIEExecutor`, keep_alive + executor delegate + Slate pre-tick). Canonical MCP entry: **`capture_shotlist.py`**. Enable **MovieRenderPipeline** / **MovieRenderPipelineEditor** / **SequencerScripting** in `HomeWorld.uproject`. **Gap OPEN** — DESKTOP must re-prove MRQ path with **non-black** stills; do not claim PASS from cloud.
