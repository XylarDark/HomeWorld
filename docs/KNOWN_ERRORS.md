...
**PA-E AutomationLibrary pretick writes near-black PNGs — post-#166 DESKTOP FAIL (2026-09-22, after ee32888):** MCP **`capture_shotlist_viewport.py`** pretick path **responsive**; report **`ok: false`** — Shot1 ~38KB **mean luminance ~0.0**, Shot2 ~143KB **mean ~0.3**. **Lead clarification:** homestead **geometry/lighting are visible** when rotating the viewport — this is an **OPEN viewport capture bug** (wrong **game-view**, **camera pilot**, **pose**, or **empty/unlit HighResShot buffer**), **not** “scene is black.” **Do not** abandon luminance/quality gates or treat near-black as acceptable. **Primary prove path:** [capture_shotlist_mrq.py](../Content/Python/capture_shotlist_mrq.py) (MRQ one-frame, in-level CAMs, deferred lit pass, warm-up). **Diagnostic:** keep fixing AL path in [capture_shotlist_viewport.py](../Content/Python/capture_shotlist_viewport.py). Gap **OPEN** until DESKTOP **lit non-black** stills — file-exists-only is **not** PASS.
