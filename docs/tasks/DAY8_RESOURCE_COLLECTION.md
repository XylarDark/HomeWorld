...
**Goal:** When the player presses Interact (E) in front of a resource pile (e.g. BP_HarvestableTree), grant Wood (or other resource) to the existing inventory subsystem. GA_Interact remains the trigger; C++ provides trace + harvest; Blueprint invokes it. See [30_DAY_SCHEDULE.md](../workflow/30_DAY_SCHEDULE.md) Day 8.
