...
- **Script:** Run in Editor with target level open (DemoMap or planetoid): **place_dungeon_entrance.py**. Idempotent: places **AHomeWorldDungeonEntrance** at `dungeon_entrance_position` from [Content/Python/dungeon_map_config.json](../../Content/Python/dungeon_map_config.json) with **LevelToOpen** from `dungeon_level_name` (e.g. `Dungeon_Interior`) and tag **Dungeon_POI**. Overlap in PIE opens the configured level. Falls back to cube if C++ class unavailable (add Open Level in Blueprint).
- **C++ trigger:** **AHomeWorldDungeonEntrance** ([Source/HomeWorld/HomeWorldDungeonEntrance.h](../../Source/HomeWorld/HomeWorldDungeonEntrance.h)) opens a level when the player overlaps its box. The script sets LevelToOpen from config; optionally create **BP_DungeonEntrance** and set **Require Pawn Tag** (e.g. `Player`) in Editor.
