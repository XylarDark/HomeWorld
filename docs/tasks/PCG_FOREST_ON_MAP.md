...
**Status:** Script tags the Landscape and creates/sizes one PCG Volume (**PCG_Forest**). Generation is manual: create (or copy) a PCG graph, set Get Landscape Data to By Tag + `PCG_Landscape`, assign the graph to the volume, click Generate.
...
**Config:** `Content/Python/demo_map_config.json` (level, volume bounds); `Content/Python/pcg_forest_config.json` (optional, for script volume sizing).
