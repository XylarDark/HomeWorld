...
**Goal:** Place resource nodes on DemoMap using **trees as the resource gathering object**: a harvestable tree Blueprint (same mesh as PCG trees, based on `AHomeWorldResourcePile`) with Wood resource type. Day 8 will add the player harvest interaction (Interact key → GAS/inventory). See [30_DAY_SCHEDULE.md](../workflow/30_DAY_SCHEDULE.md) Day 7 and [AGENTIC_BUILDING.md](AGENTIC_BUILDING.md) section 2.3 (BP_WoodPile) for Week 2 Smart Object / HarvestWood.
