...
**Goal:** Verify GetPlacementHit / GetPlacementTransform for build/placement, then run the Week 1 playtest (explore → fight → build). See [30_DAY_SCHEDULE.md](../workflow/30_DAY_SCHEDULE.md) Day 3 and [VISION.md](../workflow/VISION.md).
...
From [VISION.md](../workflow/VISION.md): **First playable loop = explore → fight → build.** Success = survive 3 beats: **crash → scout → boss → claim home** (no family/co-op yet).
...
- Check off Day 3 in [30_DAY_SCHEDULE.md](../workflow/30_DAY_SCHEDULE.md).
- Update [DAILY_STATE.md](../workflow/DAILY_STATE.md): Yesterday = Day 3 work; Today = Day 4.
