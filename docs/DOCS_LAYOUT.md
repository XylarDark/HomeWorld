...
| [Automation/CAPTURE_REDUNDANCY.md](Automation/CAPTURE_REDUNDANCY.md) | **Universal** tooling redundancy + practice (Lead-gated): **procedure-agnostic / track-agnostic evidence protocol** for bot-company workflow (Design→Test→Fix on any track); docs-first, proven-results, dead-end research, rung ladder; shotlist/PA-E instance; ImageGrab ban. |
