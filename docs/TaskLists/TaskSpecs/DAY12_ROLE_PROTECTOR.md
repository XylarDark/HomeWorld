...
**See also:** [30_DAY_SCHEDULE.md](../30_DAY_SCHEDULE.md) Day 12, [FAMILY_AGENTS_MASS_STATETREE.md](FAMILY_AGENTS_MASS_STATETREE.md), [DAY11_FAMILY_SPAWN.md](DAY11_FAMILY_SPAWN.md). **Astral death (night combat):** When the player's astral body is defeated at night, they return to body and wake at dawn — see [ASTRAL_DEATH_AND_DAY_SAFETY.md](ASTRAL_DEATH_AND_DAY_SAFETY.md).
...
Defend-at-night aligns with [VISION.md](../../../VisionBoard/Core/VISION.md) § Vanquishing foes: we **convert** attackers (strip sin → loved form), not kill them. When Defend phase is active (night), the game logs **"Defend active — convert attackers (VISION: convert, not kill)."** (once per night in `TryLogDefendPhaseActive`). When a foe is defeated during Defend, call **`AHomeWorldGameMode::ReportFoeConverted(Foe)`** so the game records the conversion (increments `ConvertedFoesThisNight`, optional role assignment). See GameMode `ReportFoeConverted`, `GetConvertedFoesThisNight`, and `EConvertedFoeRole`; full conversion gameplay (strip sin, spawn loved form) is implemented in later lists.
...
- Update [DAILY_STATE.md](../../../VisionBoard/DAILY_STATE.md): Yesterday = Day 12 (Protector); Today = Day 13 (Healer). Append [SESSION_LOG.md](../SESSION_LOG.md).
