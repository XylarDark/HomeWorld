...
**Assert (post-#170 + P1):** **`validate_png`** (luminance) then **`finalize_shot_validation`** (wide anchor / **`aim_ok`** / ray AABB). Luminance PASS ≠ framing PASS. **`shot_pair_validation`** rejects identical scrap. **`closed_fail`** when proved wrong (void after **`visible_sky_stack_ok`**, aim miss with lit scrap). Conductor **`conductor_mrq_capture_preflight`** before MRQ `main()`: Markers world, VNP module reload, **`reset_mrq_session_guards()`**. Night gate prefers **`visible_sky_stack_ok`** over bare **`stack_ok`** on MRQ PIE path.
