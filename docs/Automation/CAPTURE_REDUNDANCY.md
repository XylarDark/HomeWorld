...
**Assert (post-#170):** MRQ/viewport PASS requires **`validate_png`** in [pa_e_shotlist_common.py](../../Content/Python/pa_e_shotlist_common.py): global mean **and** center-crop mean **and** minimum fraction of pixels with **L&gt;8** / **L&gt;1** (blocks ~6% edge speckle on ~93% black); **`shot_pair_validation`** rejects identical/near-identical Shot1/Shot2. Wrong/black stills ⇒ **`capture_pass: false`**, **`prove_loop_status: in_progress`**, **`closed_fail: false`** — fix Arrange (inventory, **per-shot `aim_bounds`**, lighting) before claiming success. DESKTOP re-prove on current black stills should **fail assert** until framing/lighting fixed.

**MRQ PIE lighting (docs-first):** `MoviePipelinePIEExecutor` renders a **PIE** world. Editor-session TMP fixtures under `VS_MVP/TMP_PA_E_Arrange` may not match MRQ exposure until `reseed_pa_e_tmp_night_fixtures()` + `apply_homestead_night_tune()` run after load and **again before each MRQ job** (`reapply_night_environment_for_mrq_shot`). If `stack_ok` in Editor but MRQ stills dark, verify deferred lit pass, warm-up counts, and Epic MRQ PIE / exposure settings — **do not switch to day**. See `mrq_pie_lighting_note` in arrange gate / capture report.
