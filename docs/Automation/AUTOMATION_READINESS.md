...
| 3 | **Set UE_EDITOR** | Set the environment variable to your UnrealEditor.exe path. Example (PowerShell, current session): `$env:UE_EDITOR = "C:\Program Files\Epic Games\UE_5.8\Engine\Binaries\Win64\UnrealEditor.exe"`. To set permanently: System Properties → Environment Variables → New (user or system) → Name: `UE_EDITOR`, Value: full path to UnrealEditor.exe. | Run `.\Tools\Check-AutomationPrereqs.ps1` (recommended). It uses **Test-UE_EDITORSet** from Tools/Common-Automation.ps1 so the path is never passed when unset. Never call `Test-Path -LiteralPath $env:UE_EDITOR` without a null check; see docs/KNOWN_ERRORS.md. |
...
- Full build and UE automation in CI require a self-hosted Windows runner with UE 5.8 (ci.yml).
