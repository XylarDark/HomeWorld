...
| **Fully automated loop** | Ready | RunAutomationLoop.ps1 + NEXT_SESSION_PROMPT.md + 30_DAY_IMPLEMENTATION_STATUS.md. Requires Cursor Agent CLI. The loop auto-launches the Editor before the first round when UE_EDITOR is set; use `-NoLaunchEditor` to skip. |
...
| 3 | **Set UE_EDITOR** | Set the environment variable to your UnrealEditor.exe path. Example (PowerShell, current session): `$env:UE_EDITOR = "C:\Program Files\Epic Games\UE_5.7\Engine\Binaries\Win64\UnrealEditor.exe"`. To set permanently: System Properties → Environment Variables → New (user or system) → Name: `UE_EDITOR`, Value: full path to UnrealEditor.exe. | Run `.\Tools\Check-AutomationPrereqs.ps1` (recommended). Or in PowerShell: `if ($env:UE_EDITOR) { Test-Path -LiteralPath $env:UE_EDITOR } else { $false }` — must be True; if you see "Cannot bind argument to parameter 'LiteralPath' because it is null", UE_EDITOR is not set. |
