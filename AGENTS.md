...
- **Start all agents (one script):** Run **`.\Tools\Start-AllAgents.ps1`** to start the full company (Developer + Fixer + Guardian via Watcher); installs CLI if needed, auto-launches Editor when UE_EDITOR set.
- **Watcher (fix-on-failure):** Run **`.\Tools\Watch-AutomationAndFix.ps1`** to run the automation loop and, when it fails, start the **Fixer**; if the same failure repeats, the watcher invokes the **Guardian**. See docs/AUTOMATION_LOOP_UNTIL_DONE.md.
...
- **Automatic development cycle:** Use the **start-automation-cycle** command (or prompt: "Start automation cycle. Desires: …"). Task list: [docs/workflow/CYCLE_TASKLIST.md](docs/workflow/CYCLE_TASKLIST.md); state: [docs/workflow/CYCLE_STATE.md](docs/workflow/CYCLE_STATE.md). Say "Continue" to run the next task or retry. When the cycle is active, follow [.cursor/rules/19-automation-cycle.mdc](.cursor/rules/19-automation-cycle.mdc). See [docs/FULL_AUTOMATION_RESEARCH.md](docs/FULL_AUTOMATION_RESEARCH.md) §7.
- **Loop until all 30 days done:** The agent cannot start a new session by itself. To run session-after-session until every day is implementation-complete: each session writes [docs/workflow/NEXT_SESSION_PROMPT.md](docs/workflow/NEXT_SESSION_PROMPT.md); you (or external tooling) start the next chat and paste that prompt. Implementation status: [docs/workflow/30_DAY_IMPLEMENTATION_STATUS.md](docs/workflow/30_DAY_IMPLEMENTATION_STATUS.md). See [docs/AUTOMATION_LOOP_UNTIL_DONE.md](docs/AUTOMATION_LOOP_UNTIL_DONE.md).
