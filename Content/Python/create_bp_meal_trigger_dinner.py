# create_bp_meal_trigger_dinner.py
# Run from Unreal Editor: Tools -> Execute Python Script or via MCP execute_python_script.
# Creates BP_MealTrigger_Dinner (Blueprint child of StaticMeshActor) in /Game/HomeWorld/Building/.
# Sets default mesh to Cube and tag "Dinner" so it is visible and interactable (E). Idempotent.
# For overlap trigger: add HomeWorldMealTriggerComponent in Editor or MCP, set MealType = Dinner.
# MVP full scope List 57 T4 (in-world dinner trigger). See CONSOLE_COMMANDS.md; MVP_TUTORIAL_PLAN List 7.
...
def _log(msg):
    unreal.log("MealTrigger Dinner: " + str(msg))
    print("MealTrigger Dinner: " + str(msg))
...
def main():
    ...
    _set_defaults(bp)
    unreal.EditorAssetLibrary.save_loaded_asset(bp)
    _log("Done. Run place_meal_triggers.py with DemoMap open to place. For overlap, add HomeWorldMealTriggerComponent (MealType=Dinner) via MCP or Editor.")


def _set_defaults(bp):
    """Set default static mesh to Cube and tag 'Dinner' on the Blueprint CDO."""
    gen_class = None
    try:
    ...
    try:
        ...
        if tags is not None:
            ...
            if DINNER_TAG not in tag_strs:
                tags.append(unreal.Name(DINNER_TAG))
                if hasattr(cdo, "set_editor_property"):
                    cdo.set_editor_property("tags", tags)
                _log("Set default tag '" + DINNER_TAG + "'.")
    except Exception as e:
