# create_bp_meal_trigger_breakfast.py
# Run from Unreal Editor: Tools -> Execute Python Script or via MCP execute_python_script.
# Creates BP_MealTrigger_Breakfast (Blueprint child of StaticMeshActor) in /Game/HomeWorld/Building/.
# Sets default mesh to Cube and tag "Breakfast" so it is visible and interactable (E). Idempotent.
# For overlap trigger: add HomeWorldMealTriggerComponent in Editor or MCP add_component_to_blueprint
# (blueprint_name=BP_MealTrigger_Breakfast, component_type=HomeWorldMealTriggerComponent, component_name=MealTrigger).
# MVP full scope List 57 T2 (in-world breakfast trigger). See CONSOLE_COMMANDS.md § Tutorial (List 3).
...
BP_NAME = "BP_MealTrigger_Breakfast"
...
def _log(msg):
    unreal.log("MealTrigger Breakfast: " + str(msg))
    print("MealTrigger Breakfast: " + str(msg))
...
def main():
    ...
    _set_defaults(bp)
    unreal.EditorAssetLibrary.save_loaded_asset(bp)
    _log("Done. Run place_meal_triggers.py with DemoMap open to place. For overlap, add HomeWorldMealTriggerComponent via MCP or Editor.")


def _set_defaults(bp):
    """Set default static mesh to Cube and tag 'Breakfast' on the Blueprint CDO."""
    gen_class = None
    try:
    ...
    try:
        ...
        if tags is not None:
            ...
            if BREAKFAST_TAG not in tag_strs:
                tags.append(unreal.Name(BREAKFAST_TAG))
                if hasattr(cdo, "set_editor_property"):
                    cdo.set_editor_property("tags", tags)
                _log("Set default tag '" + BREAKFAST_TAG + "'.")
    except Exception as e:
